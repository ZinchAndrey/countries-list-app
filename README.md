...
## Ответы Александра
1.3 - если честно, я лагов не заметил (так же выкачал себе твой проект с репы, все работает быстро). Но я бы сделал немного иначе. Вот в data у тебя лежит searchTerm.
Я бы туда закидывал значение из геттера при вводе в input и очищал бы эту переменную при событии clearSearchField. А computed searchTerm() я бы убрал. Возможно, из-за этого баги.
А еще может быть такое, что при зажатии backspace происходит много запросов и операций одновременно. В таком случае ты можешь в методе search() сделать небольшой timeout. Делать поиск не при каждом измении в input, а через секунду, например. Так, кстати, работают каталоги в больших интернет-магазинах. Они отправляют запросы тогда, когда юзер перестает печатать

3. Ссылка в шапке должна быть сделано не через <a>, а через <router-link>. Тогда все будет нормально работать)


---------------

1.3 Сделал так, как ты советовал + добавил задержку по поиску, думаю действительно, теперь работа компонента более оптимизирована)
3. Действительно, как я сам не заметил(